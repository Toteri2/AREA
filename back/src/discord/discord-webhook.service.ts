...
export class DiscordWebhookService {
    ...
    async createWebhook(channelId: string, webhookUrl: string) {
        try {
            ...
            this.logger.log(`Webhook created for channel ${channelId}: ${webhook.id}`);

            return webhook;
        } catch (error) {
            this.logger.error(`Failed to create webhook for channel ${channelId}:`, error);
            throw error;
        }
    }
    ...
    async listChannelWebhooks(channelId: string) {
        try {
            ...
            this.logger.error(`Failed to list webhooks for channel ${channelId}:`, error);
            throw error;
        }
    }
    ...
    verifyWebhookSignature(signature: string, timestamp: string, body: string): boolean {
        this.logger.warn('Webhook signature verification not implemented - accepting all requests');
        return true;
    }
}
